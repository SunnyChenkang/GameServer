...
inline uint64 GetIDX(uint64 id)	{	return id>>4 & 0xFFFFFFFFFFFFFLL;	}

void test_id_gen()
{
    cout<<"---------------  test_id_gen ----------------"<<endl;
    uint16 region = 0xf;
    uint8  service_id = 0xff;

    uint64    m_BaseCode;
    uint64    m_CurrentID = 0x11111111111111LL;

    m_BaseCode  = service_id;
    m_BaseCode  = m_BaseCode << (56);
    m_BaseCode |= region & 0xf;
...
void test_eval_expr()
{
    CSString    str = "min(int(($a/5+$b)*10),70)";
    CEvalExpr   Expr;
...
void test_hashset()
{
    ...
    for (int i = 0; i < PLYNUM; ++i)
    {
        ...
        char str[64] = { '\0' };
        smprintf( str , sizeof( str ) , "%d" , nindex );
        m_hashSet.insert(str);
    }

    sint64 Curtime = NLMISC::CTime::getLocalTime();

    printf( " test_set time = %lld " , Curtime -  lasttime );
}
