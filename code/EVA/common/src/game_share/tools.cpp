...
const char* CLocalTime::printdaytime(tm& tms)
{
	std::strftime (m_cstime, TIME_STR_MAX, "%Y%m%d", &tms);
    return m_cstime;
}

const char* CLocalTime::printdaytime(const uint32 time)
{
	tm t = gettime(time);
    return 	printdaytime(t);
}
...
tm CLocalTime::AnalysisTime( CSString str_time )
{
    tm  tms;
    tms.tm_mday = -1;
    tms.tm_mon  = -1;
    ...
    CVectorSString  vct_date_time;
    CVectorSString  vct_date;
    CVectorSString  vct_time;

    str_time.splitBySeparator(' ', vct_date_time);


    if ( vct_date_time.size() == 2 )
    {
        vct_date_time[0].splitBySeparator('-', vct_date);
        vct_date_time[1].splitBySeparator(':', vct_time);
    }
    else if ( vct_date_time.size() == 1 )
    {
        vct_date_time[0].splitBySeparator(':', vct_time);
    }
    else
    {
        nlassert(0);
    ...
    nlassert( vct_date.size() == 3 );

    switch ( vct_date.size() )
    {
    case 3:
        tms.tm_mday = vct_date[2].atoi();
    case 2:
        tms.tm_mon = vct_date[1].atoi()-1;
    case 1:
        tms.tm_year = vct_date[0].atoi()-1900;
        break;
    default:
        break;
    }

    nlassert( vct_time.size() == 3 );
    
    tms.tm_hour = vct_time[0].atoi();
    tms.tm_min = vct_time[1].atoi();
    tms.tm_sec = vct_time[2].atoi();
...
struct CPMainRandom
{
    CPMainRandom()
    {
        _MainRandom.srand( CTime::getLocalTime() );
    }
...
sint32 GetRandom()
{
    return _MainRandom.rand();
}

sint32 GetRandom( sint16 mod )
{
    return _MainRandom.rand( mod );
}

sint32 GetRandom( sint16 min_dig, sint16 max_dig )
{
    if ( min_dig > max_dig ||
         min_dig < 0  ||
         max_dig < 0 )
...
uint32 GetUint32Random(uint32 min_dig,uint32 max_dig)
{
	if ( min_dig > max_dig )
	{
		nlwarning( "min_dig is bigger than max_dig, min_fig: %d, max_dig: %d ", min_dig, max_dig );
