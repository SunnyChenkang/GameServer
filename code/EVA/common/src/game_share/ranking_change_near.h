...
public:
	...
	*/
    ...
    void Erase( DEF::PID pid )
    {
        DEF::RANKING nRankingNew = GetIDRanking(pid);

        if ( nRankingNew > 0 )
        {
            while ( nRankingNew<DEF::RANKING(_vctScore.size()-1) )
            {
                _vctScore[nRankingNew] = _vctScore[nRankingNew+1];
                _updateMapRanking(nRankingNew);
    ...
    DEF::SCORE  GetScore( const DEF::PID nID )
    {
        DEF::SCORE score = 0;
        DEF::RANKING rnk = GetIDRanking(nID);

        if ( rnk > 0 && rnk<_vctScore.size() )
        {
            score = _vctScore[rnk].nScore;
        }
    ...
    typedef std::vector<SPlayerInfo>    TRankingPlayerInfo;

    TRankingPlayerInfo& GetRankingInfo()
    {
        return _vctScore;
    }
...
private:
	...
	*/
	...
	DEF::RANKING _downRanking( const DEF::RANKING nOldRanking, const DEF::SCORE nScore )
	{
		...
		/// 排名下移
		while ( nNewRanking<DEF::RANKING(_vctScore.size()-1) )
		{
			if (_vctScore[nNewRanking+1].nScore>nScore )
			{
				_vctScore[nNewRanking] = _vctScore[nNewRanking+1];
...
private:
    ...
    bool EraseIDRanking( DEF::PID pid )
    {
        bool ret = false;
        uint idx = HashFunctionPID(pid);
        ...
        if ( _HashIDRanking[idx].pid==pid )
        {
            _HashIDRanking[idx].pid = 0;
            _HashIDRanking[idx].ranking = 0;
            ret = true;
        }
        else
        {
