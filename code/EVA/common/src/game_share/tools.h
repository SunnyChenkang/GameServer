...
// generate a random value in [min_dig, max_dig]
sint32  GetRandom( sint16 min_dig, sint16 max_dig );

// generate a random value in [min_dig, max_dig](uint32)
...
const sint32 WEEK_DAYS      = 7;

inline bool UtcSameDay( uint32 time_seconds1, uint32 time_seconds2 )
{
    return (time_seconds1/DAY_SECONDS)==(time_seconds2/DAY_SECONDS);
}
...
public:
	...
	const char* printdaytime(tm& tms);
	const char* printdaytime(const uint32 time);

	const NLMISC::TTime  GetCurrTime()	{ return m_CurrTime; }
	void SetCurrTime( NLMISC::TTime curr_time )	{ m_CurrTime = curr_time; }

    tm AnalysisTime( NLMISC::CSString str_time );
...
static DEF::CHECK_SUM  CheckSum( google::protobuf::Message* pMessage )
{
...
inline std::vector<sint32> SplitLineToInt( NLMISC::CSString& data_line )
{
    std::vector<sint32>   slt_val;
...
inline std::string GetRandomString( uint32 str_len )
{
    std::string   rnd_str;
    rnd_str.resize( str_len, 'a' );

    for ( uint i=0; i<str_len; ++i )
    {
        char c = (char)GetRandom( 93 );
        c += 33;
        rnd_str[i] = c;
    }
...
inline uint64 ToInt64( std::string Param )
{
    NLMISC::CSString ParamStr( Param );
    return ParamStr.atoui64();
}
