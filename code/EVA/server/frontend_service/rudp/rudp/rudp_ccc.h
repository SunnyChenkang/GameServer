// RUDP发送码率控制器，实现带宽评估、发送窗口决策、RTT计算等;
// CCC限速控制器;
...
public:
    uint32_t            get_send_window_size( void ) const { return snd_cwnd_; };
    void                set_max_segment_size( uint16_t mss );

    void                set_rtt( uint32_t keep_live_rtt );
    uint32_t            get_rtt( void ) const { return (uint32_t)(rtt_ * rtt_scale_); };

    uint32_t            get_rtt_var( void ) const { return (uint32_t)( rtt_var_ * rtt_scale_ ); };

    void                add_resend( void );
