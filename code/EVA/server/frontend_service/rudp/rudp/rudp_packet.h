...
#define RUDP_EXIST_CONNECTED    0x17        //检查连接是否存在;
#define RUDP_EXIST_CONNECTED2   0x18        //检查连接是否存在返回包;
...
protected:
    virtual void Pack(BinStream& strm) const
    {
        ...
        uint16_t array_size = (uint16_t)loss_ids_.size();
        strm << array_size;
