...
RUDPSocket::RUDPSocket()
{
    ...
    last_msg_time_ = CBaseTimeValue::get_time_value().sec();

    keeplive_intnal_ = DEFAULT_KEEPLIVE;
    keeplive_count_ = 0;
    timeout_count_ = DEFAULT_TIMEOUT;
...
void RUDPSocket::cancel_keeplive_timer( void )
{
    if ( timer_keeplive_id_ != 0 )
    {
        const void* act = NULL;
        REACTOR_INSTANCE()->cancel_timer( timer_keeplive_id_ , &act );
        timer_keeplive_id_ = 0;
    }
}
...
{
    // 检查连接超时;
    if ( timer_keeplive_id_ == timer_id )
    {
        timer_keeplive_id_ = 0;
        if ( NULL != event_handler_ ) { event_handler_->rudp_exist_keeplive( rudp_id_ ); }
        return 0;
    }
...
int32_t RUDPSocket::send(const uint8_t* data, int32_t data_size)
{
    ...
    // 安全机制;
    if ( send_buffer_.get_send_window_size() >= 256 )
    {
        set_state(RUDP_FIN2_STATE);
        event_handler_->rudp_exception_event(rudp_id_);
        return -1;
...
void RUDPSocket::process(uint8_t msg_id, uint16_t check_sum, BinStream& strm, const Inet_Addr& remote_addr)
{
	...
	case RUDP_EXIST_CONNECTED:
		process_exist_keeplive(strm, remote_addr);
		break;

	case RUDP_EXIST_CONNECTED2:
		process_exist_keeplive2(strm, remote_addr);
		break;
    }
...
void RUDPSocket::process_exist_keeplive( BinStream& strm , const Inet_Addr& remote_addr )
{
    RUDP_INFO( "exist keellive from" << remote_addr << ", rudp_id = " << rudp_id_ );
    ...
    RUDPHeadPacket head;
    head.msg_id_ = RUDP_EXIST_CONNECTED2;
    head.remote_rudp_id_ = remote_rudp_id_;
    head.check_sum_ = check_sum_;
...
void RUDPSocket::process_exist_keeplive2( BinStream& strm , const Inet_Addr& remote_addr )
{
    RUDP_INFO( " exist keeplive from " << remote_addr << ", rudp_id = " << rudp_id_ );
...
void RUDPSocket::heartbeat()
{
    ...
    // 检查心跳存活;
    uint64_t cur_msg_time_ = CBaseTimeValue::get_time_value().sec();
    if ( cur_msg_time_ - last_msg_time_ >= DEFAULT_TIMEOUT )
    {
        last_msg_time_ = cur_msg_time_;
    ...
    // 补发消息;
    send_buffer_.reissue_sendwindows();
}
