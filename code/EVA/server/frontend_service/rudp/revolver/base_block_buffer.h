// 实现SOCKET的接受和发送BUFFER控制，主要针对TCP定义的收发缓冲区,缓冲区最大30M;
...
public:
    ...
    bool realloc_buffer(uint32_t size)
    {
        //剩余空间够用;
        ...
        uint32_t buffer_size = buffer_size_ * 2;
         //计算新的剩余空间是否够用;
        while(buffer_size -  data_length() < size)
        {
...
public:
    ...
    int32_t split(T& packet)
    {
        ...
        //缓冲区太小了;
        else if(this->data_length() < packet_size  + sizeof(uint32_t) && this->remaining_length() <= 0) 
        {
