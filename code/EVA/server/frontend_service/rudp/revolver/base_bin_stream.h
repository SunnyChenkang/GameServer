...
#include <string>
 ...
 //10M;
#define MAX_STREAM_STIRNG_SIZE 10485760
...
public:
    ...
    uint8_t*    get_wptr() { return wptr_; };
    const uint8_t* get_rptr() const { return rptr_; };

    //获取缓冲区的大小;
    uint32_t    size() const { return size_; };
    //获取数据填充区的大小;
    uint32_t    data_size() const { return used_; };

    _MyBint& operator<<(bool val)     { ENCODE(val, bool); return (*this);  };
    _MyBint& operator>>(bool& val)    { DECODE(val, bool); return (*this);  };

    _MyBint& operator<<(int8_t val)   { ENCODE(val, int8_t);return (*this); };
    _MyBint& operator>>(int8_t& val)  { DECODE(val, int8_t);return (*this); };

    _MyBint& operator<<(int16_t val)  { ENCODE(val, int16_t); return (*this); };
    _MyBint& operator>>(int16_t& val) { DECODE(val, int16_t); return (*this); };

    _MyBint& operator<<(int32_t val)  { ENCODE(val, int32_t); return (*this); };
    _MyBint& operator>>(int32_t& val) { DECODE(val, int32_t); return (*this); };

    _MyBint& operator<<(int64_t val)  { ENCODE(val, int64_t); return (*this); };
    _MyBint& operator>>(int64_t& val) { DECODE(val, int64_t); return (*this); };

    _MyBint& operator<<(uint8_t val)  { ENCODE(val, uint8_t); return (*this); };
    _MyBint& operator>>(uint8_t& val) { DECODE(val, uint8_t); return (*this); };

    _MyBint& operator<<(uint16_t val) { ENCODE(val, uint16_t); return (*this); };
    _MyBint& operator>>(uint16_t& val){ DECODE(val, uint16_t); return (*this); };

    _MyBint& operator<<(uint32_t val) { ENCODE(val, uint32_t); return (*this); };
    _MyBint& operator>>(uint32_t& val){ DECODE(val, uint32_t); return (*this); };

    _MyBint& operator<<(uint64_t val) { ENCODE(val, uint64_t); return (*this); };
    _MyBint& operator>>(uint64_t& val){ DECODE(val, uint64_t); return (*this); };
