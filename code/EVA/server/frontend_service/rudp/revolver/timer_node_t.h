...
public:
   ...
   ~BaseTimerNode_T();
    ...
    HANDLER&    get_handler() { return handler_; };
    void        set_handler(HANDLER& handler) { handler_ = handler; };

    const void* get_act() const    { return act_; };
    void        set_act(void* act) { act_ = act;  };

    uint32_t    get_internal() const { return internal_; };
    void        set_internal(uint32_t internal) { internal_ = internal; };

    uint32_t    get_timer_id() const { return timer_id_; };
    void        set_timer_id(uint32_t timer_id) { timer_id_ = timer_id; }

    void        set_time_stamp(uint32_t stamp) { timeout_stamp_ = stamp; }
    uint32_t    get_time_stamp() const { return timeout_stamp_; }
